...
-  Each sample within a batch has its own individual loss, calculated from the difference between the model's prediction and the actual value. At the end of batch processing, all these individual losses are summed, and then the arithmetic mean is calculated. The result of this mean is the final loss that we know and use for backpropagation, allowing the weight adjustment to be stable and representative for the entire data set. The mathematical representation of this process is given by:
...
-  Each sample within a batch has its own individual loss, calculated from the difference between the model's prediction and the actual value. At the end of batch processing, all these individual losses are summed, and then the arithmetic mean is calculated. The result of this mean is the final loss that we know and use for backpropagation, allowing the weight adjustment to be stable and representative for the entire data set
- Every situation have a different type of error that matters more:
...
> Next, we'll see some classification losses; however, some models may offer their own classification version, such as Ridge and ElasticNet, but they lack an activation function and are somewhat of a 'workaround' that rounds off the 'y' value

### 1. BINARY LOG-LOSS (CROSS-ENTROPY)
**MATH FUNCTION:**
$$L(y, \hat{p}) = -(y \log(\hat{p}) + (1 - y) \log(1 - \hat{p}))$$

- **TYPE:** Binary Classification.
- **DEFINITION:** It measures the performance of a classification model where the prediction input is a probability value between 0 and 1. It heavily punishes predictions that are confident and wrong.
- **WHEN TO USE:** Standard choice for Logistic Regression and Neural Networks. Use when you want the model to be "sure" about its classifications.
- **EXAMPLE:** 
```

The correct predict it's True, and the activation function return that list

[False: 0.1, True: 0.9]

So the correct predict it's 0.9 or 90% of assurance, so the loss will be (-Log(0.7))
```
- **HANDS-ON**:
    - SKLearn: `log_loss()`
...
### 2. MULTI-CLASS LOG-LOSS (CATEGORICAL CROSS-ENTROPY)
**MATH FUNCTION:**
$$L = -\sum_{i=1}^{N} y_{i,c} \log(\hat{p}_{i,c})$$

- **TYPE:** Multi-class Classification.
- **DEFINITION:** It calculates the negative log of the probability assigned to the **correct** class. 
    - *Example:* If the target is [Apple: 0, Banana: 1, Orange: 0] and the model predicts [0.1, 0.7, 0.2], the loss is based on $-\log(0.7)$.
- **WHEN TO USE:** When you have more than two classes (e.g., Image recognition, species classification).
- **EXAMPLE:** 
```
The correct predict it's BANANA, and the activation function return that list

[False: 0.1, Banana: 0.7, Orange: 0.2]

So the correct predict it's 0.7 or 70% of assurance, so the loss will be (-Log(0.7))
```
- **HANDS-ON**:
    - Models: `XGBClassifier()`, `LGBMClassifier()`, `GradientBoostingClassifier()`.
...
- **TYPE:** Classification (mostly Binary).
- **DEFINITION:** It provides a "margin." If the prediction is correct and far from the decision boundary (margin > 1), loss is 0. It only starts punishing if the point is too close to the boundary or on the wrong side.
- **WHEN TO USE:** When you want a robust maximum-margin classifier. It is less sensitive to outliers than Log-Loss.
- **EXAMPLE:**
```
The others losses try to push the probabilites for 1 (100%), but the hinge have a little border, that if the assurance is far away from the border and in the right side, the loss it's 0.

[0.2, 0.8] 

The right class it's the 0.8, and the border was 0.4-0.6, so in this case the loss it's 0.
```
- **HANDS-ON**:
...
- **TYPE:** Classification.
- **DEFINITION:** It uses Euler's number ($e \approx 2.718$) raised to the negative product of the label and the prediction. It penalizes errors exponentially, making it very aggressive.
- **WHEN TO USE:** Primarily used in **AdaBoost**. It is very sensitive to outliers because it forces the model to focus heavily on previously misclassified points.
- **EXAMPLE:**
    - The correct class is **EDIBLE (+1)**.
    - The model is very wrong and predicts **-2.0** (confident it is Poisonous).
    - Calculation: $e^{-(1 \cdot -2.0)} = e^{2.0} \approx 7.38$.
    - **Loss = 7.38** (An aggressive penalty to force the next "weak learner" to fix this error).
- **HANDS-ON**:
...
- **TYPE:** Classification (Deep Learning / Object Detection).
- **DEFINITION:** An evolution of Log-Loss that adds a "modulating factor" $(1 - p_t)^\gamma$. This down-weights "easy" examples and forces the model to focus on **hard, misclassified examples**.
- **WHEN TO USE:** When you have massive **class imbalance** (e.g., detecting rare diseases or small objects in large images).
- **HANDS-ON**:
    - Implementation: Custom loss functions in `PyTorch` or `TensorFlow`.
