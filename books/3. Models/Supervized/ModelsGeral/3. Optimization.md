Now that we understanding how good or bad the predict of a model are with the loss function, we need to know how the model deals with that, so after the loss function give us a number, how the model does with that?


ADD

HYPERPARAMETERS: LR (ALPHA), EPOCH AND ITERATION
TYPE OF GRADIENTS: BATCH, SGDM MINI BATCH
ORDER OF MODEL: FORWARDPASS, LOSS, GRADIENT, LR, OPTIMIZER, ITERATIONS, LOSS
GRADIENT: COMPASS (DEFINE - OR + FOR PARAMETERS) (WHAT RECIBES AND WHATS RETURNS)
